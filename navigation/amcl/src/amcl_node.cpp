...
static double
normalize(double z)//将角度范围限制在-pi~pi
{
...
main(int argc, char** argv)
{
	...
	if (argc == 1)
	{
		// run using ROS input，我们这个项目肯定是直接用gros输入
		ros::spin();
	}
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
	...
	initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);//订阅初始位姿
}
