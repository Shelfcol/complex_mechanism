	...
	first_reconfigure_call_(true)
{
	...
	pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true);//发送全局定位信息给robot
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
	...
	initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);//���ĳ�ʼλ��
}
...
//收到激光雷达数据之后进行amcl的计算，然后将定位结果发给robot
void
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
	ROS_INFO("！!！!！!！!！!！!！!！!！!！!！!！!！!amcl receive laser");
	...
	if (resampled || force_publication)
	{
		...
		if (max_weight > 0.0)
		{
			 ...
			 */
			ROS_INFO("！!！!！!！!！amcl node   x=%d, y=%d, z=%d", p.pose.pose.position.x, p.pose.pose.position.y, p.pose.pose.position.z);
